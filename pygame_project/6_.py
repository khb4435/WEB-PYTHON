...
# 화면 타이틀 설정
pygame.display.set_caption("연습게임")
...
# 무기는 한 번에 여러 발 발사 가능 --> "리스트로 처리"
weapons = []

# 무기 이동 속도
weapon_speed = 10

# 공 만들기 (4개 크기에 대해 따로 처리) #<<--------------------------------------"리스트"로 4개를 한 묶음 처리!
ball_images = [
...
# 공들
balls = [] #<<-------여러개의 공들정보가 들어갈 거라 "리스트"로 표현했지만 각 마다 속성을 표시해주려고 "딕셔너리"함
...
while running:
    ...
    # 2. 이벤트 처리 (키보드, 마우스 등)
    for event in pygame.event.get(): #get에 쌓인만큼만 반복문 돌리고 빠져나가는듯??
        #print("for문")
    ...
    # 천장에 닿은 무기 없애기 //???????????????????????????????????????????????????????????????????????????????????????? w[1]을 - 시키면서 업데이트하는 코드
